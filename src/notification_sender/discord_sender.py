...
class DiscordSender:
    ...
    def send_to_discord(
        ...
        if self._discord_config["webhook_url"]:
            return all(
                self._send_discord_webhook(chunk, timeout_seconds=timeout_seconds)
                for chunk in chunks
            )
        ...
        if self._discord_config["bot_token"] and self._discord_config["channel_id"]:
            return all(
                self._send_discord_bot(chunk, timeout_seconds=timeout_seconds)
                for chunk in chunks
            )
    ...
    def _send_discord_webhook(
        ...
        Returns:
            是否发送成功
        """
        webhook_url = self._discord_config["webhook_url"]
        if not webhook_url:
            logger.warning("DISCORD_WEBHOOK_URL 未配置，跳过 Discord 推送")
            return False
        try:
            payload = {
                "content": content,
            ...
            if response.status_code in [200, 204]:
                ...
                logger.error(
                    f"Discord Webhook 发送失败: {response.status_code} {response.text}"
                )
                return False
    ...
    def _send_discord_bot(
        ...
        try:
            ...
            response = requests.post(
                url, json=payload, headers=headers, timeout=timeout_seconds or 10
            )
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Discord Bot 发送失败: {response.status_code} {response.text}"
                )
                return False
        except Exception as e:
