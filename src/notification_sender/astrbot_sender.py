...
class AstrbotSender:
    ...
    def _send_astrbot(
        ...
        astrbot_url = self._astrbot_config["astrbot_url"]
        if not astrbot_url:
            logger.warning("ASTRBOT_URL 未配置，跳过 AstrBot 推送")
            return False
        ...
        try:
            ...
            if self._astrbot_config["astrbot_token"]:
                ...
                signature = hmac.new(
                    key.encode("utf-8"), sign_data, hashlib.sha256
                ).hexdigest()
            url = astrbot_url
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"AstrBot 发送失败: {response.status_code} {response.text}"
                )
                return False
