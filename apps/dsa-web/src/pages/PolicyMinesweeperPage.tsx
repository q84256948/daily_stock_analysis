...
import { usePolicyMinesweeper } from '../hooks/usePolicyMinesweeper';
import {
  policyMinesweeperApi,
  type PolicyMinesweeperHorizon,
  type PolicyMinesweeperReportItem,
...
/** 时间窗口选项（与后端 PolicyMinesweeperRequest.horizon 对齐）。 */
const HORIZON_OPTIONS: { value: PolicyMinesweeperHorizon; label: string; hint: string }[] = [
  { value: 'short', label: '短期', hint: '1-5 日' },
  { value: 'medium', label: '中期', hint: '1-4 周' },
  { value: 'long', label: '长期', hint: '1-6 月' },
];

/**
 * 政策与公告双维度排雷页面（表单流：输入股票 → SSE 生成 → 报告展示 + PDF + 历史）。
 *
 * 布局同 DeepResearchPage 双栏：左栏历史报告列表（desktop w-64 / mobile Drawer），
 * 右栏表单 + 报告/进度/错误。差异：
 * - 时间窗口选择器（short/medium/long，影响 α/β 权重）。
 * - 三角色（α 公告 / β 政策 / Ω 综合）执行状态指示（哪些 Agent 成功）。
 * - 综合分/等级/置信度由报告正文 scorecard banner 呈现，不重复 badges。
 */
export function PolicyMinesweeperPage() {
  const [query, setQuery] = useState('');
  const [selectedStock, setSelectedStock] = useState<SelectedStock | null>(null);
  const [horizon, setHorizon] = useState<PolicyMinesweeperHorizon>('medium');
  const [inputError, setInputError] = useState<string | null>(null);
  const [history, setHistory] = useState<PolicyMinesweeperReportItem[]>([]);
  const [historyLoading, setHistoryLoading] = useState(false);
  const [currentDetail, setCurrentDetail] = useState<PolicyMinesweeperReportDetail | null>(null);
  const [pdfLoading, setPdfLoading] = useState(false);
  const [sidebarOpen, setSidebarOpen] = useState(false);

  const { status, progressSteps, report, reportId, error, generate, cancel, reset } =
    usePolicyMinesweeper();
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const items = await policyMinesweeperApi.getReports(50);
      setHistory(items);
    } catch {
      // 历史加载失败不阻塞主流程
    } finally {
  ...
  const handleSubmit = useCallback(() => {
    ...
    reset();
    void generate(selectedStock.code, selectedStock.name, horizon);
  }, [selectedStock, horizon, generate, reset]);

  const handleSelectHistory = useCallback(
    async (id: string) => {
      try {
        const detail = await policyMinesweeperApi.getReport(id);
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      try {
        await policyMinesweeperApi.deleteReport(id);
        await loadHistory();
  ...
  const handleDownloadPdf = useCallback(async (id: string) => {
    setPdfLoading(true);
    try {
      await policyMinesweeperApi.downloadPdf(id);
    } catch (e) {
      ...
      <div className="flex h-full min-w-0 flex-1 flex-col overflow-hidden">
        <header className="mb-4 flex-shrink-0 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-foreground">政策与公告双维度排雷</h1>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* 时间窗口选择器 */}
            <div className="inline-flex h-11 items-center rounded-xl border border-white/10 bg-white/2 p-1">
        ...
        <div className="flex-1 overflow-y-auto rounded-[1.25rem] border border-white/8 bg-card/82 p-5 shadow-soft-card">
          ...
          {isGenerating && (
            <div className="space-y-4">
              <div className="flex items-center gap-3 text-secondary-text">
                ...
                <span>
                  正在执行三角色并行排雷（α 公告扫描 + β 政策分析 + Ω 综合裁决），预计 5-9 分钟，请勿关闭页面...
                </span>
              </div>
                ...
                <div className="mt-2 max-h-60 space-y-1 overflow-y-auto text-xs text-muted-text">
                  {progressSteps.map((s, i) => (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      {s.agent && (
                        <span className="rounded bg-white/5 px-1 text-[10px] uppercase text-cyan/80">
                          {s.agent}
                        </span>
                      )}
                      <span>
          ...
          {status === 'idle' && !showReport && (
            <div className="flex flex-col items-center justify-center gap-3 py-16 text-center text-muted-text">
              ...
              <p>输入 A 股代码或名称，生成政策与公告双维度排雷报告</p>
              <p className="text-xs">三角色并行：α 公告扫描 → β 政策分析 → Ω 综合裁决</p>
            </div>
          )}
          ...
          {showReport && displayReport && (
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {/* 三角色执行状态（哪些 Agent 成功；partial 时据此判断降级来源） */}
                {displayReport.status && (
                  <span className="ml-auto flex items-center gap-2 text-[11px] text-muted-text">
                    {[
                      { key: 'alpha_ok', label: 'α公告' },
                      { key: 'beta_ok', label: 'β政策' },
                      { key: 'omega_ok', label: 'Ω综合' },
                    ].map(({ key, label }) => {
                      const ok = displayReport[key as 'alpha_ok' | 'beta_ok' | 'omega_ok'];
                      return (
                        <span
                          key={key}
              ...
              <ReportMarkdownBody
                ...
                className="policy-minesweeper-prose"
              />
            </div>
          )}
        </div>
...
export default PolicyMinesweeperPage;
