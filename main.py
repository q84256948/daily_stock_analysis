...
if os.getenv("DSA_PACKAGED_ALPHASIFT_IMPORT_PROBE") == "1":
    ...
    try:
        ...
        print(
            f"ERROR: packaged AlphaSift adapter import failed: {exc}", file=sys.stderr
        )
        sys.exit(1)
...
示例:
  ...
  python main.py --market-review    # 仅运行大盘复盘
    ...
    parser.add_argument(
        "--check-notify", action="store_true", help="只读检查通知渠道配置，不发送通知"
    )
...
def _refresh_stock_index_cache_for_analysis(config: Config) -> None:
    ...
    try:
        ...
        if result.refreshed:
            ...
            logger.debug(
                "[stock-index] 分析前刷新未完成，继续使用本地索引: %s", result.error
            )
...
def _save_reused_market_review_report(
    ...
    *,
    ...
    if not any(
        body.startswith(item) for item in ("# 🎯 大盘复盘", "# 🎯 Market Review")
    ):
        body = f"{title}\n\n{body}"
    try:
...
def run_full_analysis(
    ...
    try:
        ...
        should_use_daily_market_context = should_run_market_review and getattr(
            config, "daily_market_context_enabled", True
        )
        ...
        if should_run_market_review:
            ...
            if can_skip_market_review:
                ...
                logger.info("复盘上下文可复用，跳过重复大盘复盘并复用上下文内容。")
            ...
            if not can_skip_market_review:
                if analysis_delay > 0:
                    logger.info(
                        f"等待 {analysis_delay} 秒后执行大盘复盘（避免API限流）..."
                    )
...
def _resolve_scheduled_stock_codes(
...
def main() -> int:
    ...
    if args.stocks:
        stock_codes = [
            canonical_stock_code(c) for c in args.stocks.split(",") if (c or "").strip()
        ]
    ...
    if start_serve:
        if not prepare_webui_frontend_assets():
            logger.warning(
                "前端静态资源未就绪，继续启动 FastAPI 服务（Web 页面可能不可用）"
            )
        try:
    ...
    try:
        ...
        if args.market_review:
            ...
            effective_region = None
            if not getattr(args, "force_run", False) and getattr(
        ...
        if args.schedule or config.schedule_enabled:
            ...
            if getattr(config, "agent_event_monitor_enabled", False):
                ...
                interval_minutes = max(
                    1, getattr(config, "agent_event_monitor_interval_minutes", 5)
                )
                ...
                def event_monitor_task():
                    ...
                    if triggered_count:
                        logger.info(
                            "[EventMonitor] 本轮触发 %d 条提醒", triggered_count
                        )
            ...
            # 多任务调度：检查是否配置了独立的自选股分析时间和大盘复盘时间
            watchlist_analysis_time = getattr(
                config, "watchlist_analysis_time", ""
            ).strip()
            market_review_time = getattr(config, "market_review_time", "").strip()
            ...
            if market_review_time:
                ...
                daily_tasks.append(
                    {
                        "name": "market_review",
                        "task": market_review_task,
                        "schedule_time": market_review_time,
                        "run_immediately": should_run_immediately,
                    }
                )
