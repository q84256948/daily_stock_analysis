...
@require(
    lambda stock_code: isinstance(stock_code, str) and len(stock_code.strip()) > 0,
...
class DataFetcherManager:
    ...
    _daily_source_health = CircuitBreaker(
        failure_threshold=2,  # 优化：从 3 改为 2，更快隔离问题源
        cooldown_seconds=180.0,  # 优化：从 300 改为 180 秒
        half_open_max_calls=2,  # 新增：熔断恢复时允许 2 个测试请求
    )
    ...
    def _ensure_concurrency_guards(self) -> None:
        ...
        if (
            not hasattr(self, "_fetcher_call_locks_lock")
            or self._fetcher_call_locks_lock is None
        ):
            self._fetcher_call_locks_lock = RLock()
        if not hasattr(self, "_stock_name_cache") or self._stock_name_cache is None:
            self._stock_name_cache = {}
        if (
            not hasattr(self, "_stock_name_cache_lock")
            or self._stock_name_cache_lock is None
        ):
            self._stock_name_cache_lock = RLock()
    ...
    def _call_availability_probe(
        fetcher: BaseFetcher, probe_name: str, capability: str
    ) -> Optional[bool]:
    ...
    def _call_fetcher_method(
        self, fetcher: BaseFetcher, method_name: str, *args, **kwargs
    ):
    ...
    def _record_daily_source_failure(
        cls, fetcher: BaseFetcher, market: str, error: str
    ) -> None:
        cls._daily_source_health.record_failure(
            cls._daily_health_key(fetcher, market), error=error
        )
    ...
    def _try_scalar_isna(value: Any, context: str) -> Optional[bool]:
        ...
        if isinstance(
            value, (dict, list, tuple, set, pd.DataFrame, pd.Series, pd.Index)
        ):
            return None
        ...
        if isinstance(raw_data, pd.DataFrame):
            ...
            name_col = next(
                (
                    ...
                    if str(col)
                    in {"板块名称", "板块", "所属板块", "板块名", "name", "industry"}
                ),
                None,
            )
            code_col = next(
    ...
    def _init_default_fetchers(self) -> None:
        ...
        alphavantage_api_key = (
            getattr(config, "alphavantage_api_key", None) or ""
        ).strip()
    ...
    def get_daily_data(
        ...
        fetchers = self._filter_fetchers_by_capability(
            fetchers, capability="daily_data"
        )
        ...
        if is_us:
            prefer_lb = (
                self._longbridge_preferred(capability="daily_data") and not is_us_index
            )
            if is_us_index:
    ...
    def prefetch_realtime_quotes(self, stock_codes: List[str]) -> int:
        ...
        if not getattr(config, "prefetch_realtime_quotes", True):
            logger.debug(
                "[预取] component=realtime_prefetch action=skip reason=disabled"
            )
            return 0
        ...
        if not config.enable_realtime_quote:
            logger.debug(
                "[预取] component=realtime_prefetch action=skip reason=realtime_quote_disabled"
            )
            return 0
    ...
    def _enrich_realtime_quote(
        ...
        *,
        ...
        fetched_dt = self._parse_realtime_timestamp(fetched_at) or datetime.now(
            timezone.utc
        )
    ...
    def get_realtime_quote(self, stock_code: str, *, log_final_failure: bool = True):
        ...
        for source_index, source in enumerate(source_priority):
            attempt_start = time.time()
            fallback_to = (
                source_priority[source_index + 1]
                if source_index + 1 < len(source_priority)
                else None
            )
            fetcher = None
            try:
                ...
                if source == "efinance":
                    fetcher = self._get_fetcher_by_name(
                        "EfinanceFetcher", capability="realtime_quote"
                    )
                    if fetcher is not None and hasattr(fetcher, "get_realtime_quote"):
                        ...
                        quote = self._call_fetcher_method(
                            fetcher, "get_realtime_quote", stock_code
                        )

                elif source == "akshare_em":
                    fetcher = self._get_fetcher_by_name(
                        "AkshareFetcher", capability="realtime_quote"
                    )
                    if fetcher is not None and hasattr(fetcher, "get_realtime_quote"):
                        record_provider_run_started(
                            data_type="realtime_quote",
                        ...
                        quote = self._call_fetcher_method(
                            fetcher, "get_realtime_quote", stock_code, source="em"
                        )
                    ...
                    if fetcher is not None and hasattr(fetcher, "get_realtime_quote"):
                        ...
                        quote = self._call_fetcher_method(
                            fetcher, "get_realtime_quote", stock_code, source="sina"
                        )
                    ...
                    if fetcher is not None and hasattr(fetcher, "get_realtime_quote"):
                        ...
                        quote = self._call_fetcher_method(
                            fetcher, "get_realtime_quote", stock_code, source="tencent"
                        )

                elif source == "tushare":
                    fetcher = self._get_fetcher_by_name(
                        "TushareFetcher", capability="realtime_quote"
                    )
                    if fetcher is not None and hasattr(fetcher, "get_realtime_quote"):
                        record_provider_run_started(
                            data_type="realtime_quote",
                        ...
                        quote = self._call_fetcher_method(
                            fetcher, "get_realtime_quote", raw_stock_code or stock_code
                        )
                ...
                if quote is not None and quote.has_basic_data():
                    record_provider_run(
                        ...
                        fallback_to=fallback_to
                        if primary_quote is None and self._quote_needs_supplement(quote)
                        else None,
                        record_count=1,
                    )
                    if primary_quote is None:
                        ...
                        primary_quote = quote
                        primary_fallback_from = (
                            failed_sources[0] if failed_sources else None
                        )
                        logger.info(
                            f"[实时行情] {stock_code} 成功获取 (来源: {source})"
                        )
                        ...
                        if not self._quote_needs_supplement(primary_quote):
                            return self._enrich_realtime_quote(
                                ...
                                realtime_cache_ttl=getattr(
                                    config, "realtime_cache_ttl", None
                                ),
                            )
                        # Otherwise, continue to try later sources for missing fields
                        logger.debug(
                            f"[实时行情] {stock_code} 部分字段缺失，尝试从后续数据源补充"
                        )
                        ...
                        if merged:
                            logger.info(
                                f"[实时行情] {stock_code} 从 {source} 补充了缺失字段: {merged}"
                            )
                ...
                logger.info(
                    f"[实时行情] {stock_code} {error_msg}，继续尝试下一个数据源"
                )
                errors.append(error_msg)
                if primary_quote is None:
                    failed_sources.append(source)
        ...
        if log_final_failure:
            if errors:
                logger.info(
                    f"[实时行情] {stock_code} 所有数据源均失败: {'; '.join(errors)}"
                )
            else:
    ...
    def _try_fetcher_quote(self, stock_code: str, fetcher_name: str, **kw):
        ...
        try:
            ...
            q = self._call_fetcher_method(
                fetcher, "get_realtime_quote", stock_code, **kw
            )
    ...
    def _supplement_quote(
        self, stock_code: str, primary_quote, fetcher_name: str, **kw
    ):
        ...
        if primary_quote is not None:
            ...
            try:
                ...
                if secondary is not None:
                    ...
                    if filled:
                        logger.info(
                            f"[实时行情] {stock_code} 从 {fetcher_name} 补充了: {filled}"
                        )
            except Exception as e:
        ...
        if q is not None:
            logger.info(
                f"[实时行情] {stock_code} 从 {fetcher_name} 获取成功 (独立数据源)"
            )
        return q
    ...
    def get_chip_distribution(self, stock_code: str):
        ...
        for index, (fetcher, fetcher_name, source_key) in enumerate(candidate_fetchers):
            ...
            try:
                ...
                chip = self._call_fetcher_method(
                    fetcher, "get_chip_distribution", stock_code
                )
                latency_ms = int((time.time() - attempt_start) * 1000)
    ...
    def get_stock_name(
        self, stock_code: str, allow_realtime: bool = True
    ) -> Optional[str]:
        """
        获取股票中文名称（自动切换数据源）
        ...
        if allow_realtime:
            quote = self.get_realtime_quote(
                raw_stock_code or stock_code, log_final_failure=False
            )
            if (
                quote
        ...
        for fetcher in self._get_fetchers_snapshot():
            ...
            try:
                ...
                if is_meaningful_stock_name(name, stock_code):
                    self._cache_stock_name(stock_code, name)
                    logger.info(
                        f"[股票名称] 从 {fetcher.name} 获取: {stock_code} -> {name}"
                    )
                    return name
            except Exception as e:
        ...
        for index, fetcher in enumerate(candidate_fetchers):
            ...
            try:
                ...
                if boards:
                    ...
                    logger.info(
                        f"[{fetcher.name}] 获取所属板块成功: {stock_code}, count={len(boards)}"
                    )
                    return boards
                record_provider_run(
                    data_type="belong_boards",
    ...
    def prefetch_stock_names(
        self, stock_codes: List[str], use_bulk: bool = False
    ) -> None:
        """
        ...
        for fetcher in self._get_fetchers_snapshot():
            ...
            try:
                ...
                if stock_list is not None and not stock_list.empty:
                    ...
                    logger.info(
                        f"[股票名称] 从 {fetcher.name} 批量获取完成，剩余 {len(missing_codes)} 个待查"
                    )
            except Exception as e:
    ...
    def get_market_stats(self, *, purpose: str = "unspecified") -> Dict[str, Any]:
        """获取市场涨跌统计（自动切换数据源）"""
        logger.info(
            "[MarketStats] component=market_stats action=start purpose=%s", purpose
        )
        tickflow_fetcher = self._get_tickflow_fetcher()
        if tickflow_fetcher is not None:
        ...
        for fetcher in self._fetchers:
            ...
            try:
                ...
                continue
        logger.warning(
            "[MarketStats] component=market_stats action=complete status=empty purpose=%s",
            purpose,
        )
        return {}
    ...
    def _run_with_retry(
        ...
        for _ in range(attempts):
            ...
            result, err, cost_ms = self._run_with_timeout(
                task, remaining_seconds, task_name
            )
    ...
    def _normalize_source_chain(
        ...
        if entries is None:
            return [
                {"provider": provider, "result": result, "duration_ms": duration_ms}
            ]
    ...
    def _has_meaningful_payload(payload: Any) -> bool:
        ...
        if isinstance(payload, (pd.Series, pd.Index)):
            return any(
                DataFetcherManager._has_meaningful_payload(v) for v in payload.tolist()
            )
        if isinstance(payload, np.ndarray):
            if payload.ndim == 0:
                ...
                return any(
                    DataFetcherManager._has_meaningful_payload(v) for v in payload.flat
                )
    ...
    def _should_cache_fundamental_context(context: Any) -> bool:
        ...
        for block in (
            ...
            if isinstance(payload, dict) and DataFetcherManager._has_meaningful_payload(
                payload.get("data")
            ):
                return True
        return False
    ...
    def _build_offshore_fundamental_context(
        ...
        valuation_timeout = (
            min(fetch_timeout, stage_timeout) if stage_timeout > 0 else 0
        )
        ...
        if (
            ...
            and valuation_err
            and not self._has_meaningful_payload(valuation_payload)
        ):
        ...
        growth_payload = (
            bundle_payload.get("growth", {})
            if isinstance(bundle_payload.get("growth"), dict)
            else {}
        )
        earnings_payload = (
            bundle_payload.get("earnings", {})
            if isinstance(bundle_payload.get("earnings"), dict)
            else {}
        )
        belong_boards = (
            bundle_payload.get("belong_boards")
            if isinstance(bundle_payload.get("belong_boards"), list)
            else []
        )
        ...
        if all(value == "not_supported" for value in active_statuses.values()):
            ...
            "failed" in active_statuses.values()
            or "partial" in active_statuses.values()
        ):
    ...
    def build_failed_fundamental_context(
    ...
    def get_fundamental_context(
        ...
        growth_payload = (
            bundle_payload.get("growth", {}) if isinstance(bundle_payload, dict) else {}
        )
        earnings_payload = (
            bundle_payload.get("earnings", {})
            if isinstance(bundle_payload, dict)
        ...
        institution_payload = (
            bundle_payload.get("institution", {})
            if isinstance(bundle_payload, dict)
            else {}
        )
        ...
        if isinstance(dividend_payload, dict):
            ...
            if isinstance(quote_payload, dict):
                ...
                latest_price_raw = (
                    getattr(quote_payload, "price", None) if quote_payload else None
                )
            latest_price = None
            ...
            if ttm_yield is not None:
                dividend_payload["yield_formula"] = (
                    "ttm_cash_dividend_per_share / latest_price * 100"
                )
            earnings_payload["dividend"] = dividend_payload

        adapter_errors = (
            list(bundle_payload.get("errors", []))
            if isinstance(bundle_payload, dict)
            else []
        )
        ...
        if is_etf:
            ...
            result_ctx["status"] = (
                "not_supported"
                if all(value == "not_supported" for value in block_statuses.values())
                else "partial"
            )
        elif all(value == "not_supported" for value in block_statuses.values()):
            result_ctx["status"] = "not_supported"
        elif (
            "failed" in block_statuses.values() or "partial" in block_statuses.values()
        ):
            result_ctx["status"] = "partial"
        else:
            result_ctx["status"] = "ok"
