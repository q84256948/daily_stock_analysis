...
class EfinanceFetcher(BaseFetcher):
    ...
    priority = int(
        os.getenv("EFINANCE_PRIORITY", "0")
    )  # 最高优先级，排在 AkshareFetcher 之前
    ...
    @retry(
        stop=stop_after_attempt(3),  # 修复：增加到 3 次重试
        wait=wait_exponential(
            multiplier=0.5, min=1, max=10
        ),  # 优化：缩短等待时间，使用指数退避
    ...
    def _fetch_raw_data(
        ...
        if _is_us_code(stock_code):
            raise DataFetchError(
                f"EfinanceFetcher 不支持美股 {stock_code}，请使用 AkshareFetcher 或 YfinanceFetcher"
            )
        ...
        if _is_hk_market(stock_code):
            raise DataFetchError(
                f"EfinanceFetcher 不支持港股日线 {stock_code}，请使用 AkshareFetcher 或其他港股数据源"
            )
    ...
    def get_realtime_quote(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        ...
        try:
            ...
            logger.info(
                f"[超时] ef.stock.get_realtime_quotes() 超过 {_EF_CALL_TIMEOUT}s，跳过 {stock_code}"
            )
    ...
    def _get_etf_realtime_quote(
        ...
        try:
            ...
            if (
                ...
                logger.debug(
                    f"[缓存命中] ETF实时行情(efinance) - 缓存年龄 {cache_age}s/{_etf_realtime_cache['ttl']}s"
                )
            else:
                self._set_random_user_agent()
                self._enforce_rate_limit()
                ...
                logger.info(
                    "[API调用] ef.stock.get_realtime_quotes(['ETF']) 获取ETF实时行情..."
                )
                import time as _time
                ...
                if df is not None and not df.empty:
                    logger.info(
                        f"[API返回] ETF 实时行情成功: {len(df)} 条, 耗时 {api_elapsed:.2f}s"
                    )
                    circuit_breaker.record_success(source_key)
                else:
            ...
            if df is None or df.empty:
                logger.info(
                    f"[实时行情] ETF实时行情数据为空(efinance)，跳过 {stock_code}"
                )
                return None
    ...
    def get_main_indices(self, region: str = "cn") -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            logger.info(
                "[API调用] ef.stock.get_realtime_quotes(['沪深系列指数']) 获取指数行情..."
            )
            import time as _time
            ...
            logger.info(
                f"[API返回] 指数行情成功: {len(df)} 条, 耗时 {api_elapsed:.2f}s"
            )
            code_col = "股票代码" if "股票代码" in df.columns else "code"
            code_series = df[code_col].astype(str).str.zfill(6)
    ...
    def _calc_market_stats(
        ...
        code_col = next(
            (
                c
                for c in ["代码", "股票代码", "ts_code", "stock_code"]
                if c in df.columns
            ),
            None,
        )
        name_col = next(
            (c for c in ["名称", "股票名称", "name", "name"] if c in df.columns), None
        )
        close_col = next(
            (c for c in ["最新价", "最新价", "close", "lastPrice"] if c in df.columns),
            None,
        )
        pre_close_col = next(
            (
                c
                for c in ["昨收", "昨日收盘", "pre_close", "lastClose"]
                if c in df.columns
            ),
            None,
        )
        amount_col = next(
            (c for c in ["成交额", "成交额", "amount", "amount"] if c in df.columns),
            None,
        )
        ...
        for code, name, current_price, pre_close, amount in zip(
            ...
            limit_up_price_Tolerance = round(
                abs(pre_close * (1 + ratio) - limit_up_price), 10
            )
            limit_down_price_Tolerance = round(
                abs(pre_close * (1 - ratio) - limit_down_price), 10
            )

            # C. 精确比对
            if current_price > 0:
                is_limit_up = (current_price > 0) and (
                    abs(current_price - limit_up_price) <= limit_up_price_Tolerance
                )
    ...
    def get_sector_rankings(
        ...
        try:
            ...
            logger.info(
                "[API调用] ef.stock.get_realtime_quotes(['行业板块']) 获取板块行情..."
            )
    ...
    def get_base_info(self, stock_code: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"[API调用] ef.stock.get_base_info(stock_codes={stock_code}) 获取基本信息..."
            )
            import time as _time
            ...
            api_elapsed = _time.time() - api_start
            logger.info(
                f"[API返回] ef.stock.get_base_info 成功, 耗时 {api_elapsed:.2f}s"
            )
    ...
    def get_belong_board(self, stock_code: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.info(
                f"[API调用] ef.stock.get_belong_board(stock_code={stock_code}) 获取所属板块..."
            )
            import time as _time
            ...
            logger.warning(
                f"[超时] ef.stock.get_belong_board({stock_code}) 超过 {_EF_CALL_TIMEOUT}s，跳过"
            )
            return None
        except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if quote:
            print(
                f"[实时行情] {quote.name}: 价格={quote.price}, 涨跌幅={quote.change_pct}%"
            )
        else:
            print("[实时行情] 未获取到数据")
    except Exception as e:
    ...
    try:
        ...
        if info:
            print(
                f"[基本信息] 市盈率={info.get('市盈率(动)', 'N/A')}, 市净率={info.get('市净率', 'N/A')}"
            )
        else:
            print("[基本信息] 未获取到数据")
    except Exception as e:
    ...
    try:
        ...
        if stats:
            ...
            print(
                f"Down: {stats['down_count']} (Limit Down: {stats['limit_down_count']})"
            )
